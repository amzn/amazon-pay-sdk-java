...
public class PaymentsConfigTest {
    ...
    public void testPaymentsConfig() {
        PaymentsConfig config = new PaymentsConfig(TestConstants.merchantId, TestConstants.accessKey, 
                TestConstants.secretKey).withCurrencyCode(CurrencyCode.USD).sandboxMode(false).withRegion(Region.US)
    ...
    private void assertConfigValues(PaymentsConfig config) {
        Assert.assertEquals(config.getAccessKeyId() , TestConstants.accessKey);
        Assert.assertEquals(config.getSecretAccessKey() , TestConstants.secretKey);
