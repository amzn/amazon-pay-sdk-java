...
/**
 * 
 * Get Order Reference Details Samples
 * 
 * 
 */
public class GetOrderReferenceDetailsSample {
	...
	/**
	 * Just add few required parameters, and try the service Get Order Reference
	...
	public static void main(String... args) {
		...
		OffAmazonPaymentsService service = new OffAmazonPaymentsServiceClient(
				config);

		/**********************************************************************
		 * Setup request parameters and try out sample for Get Order Reference
		 * Details
		 ***********************************************************************/
		...
		GetOrderReferenceDetailsResponse response = invokeGetOrderReferenceDetails(
				service, request, outStream);
		printResponse(response, outStream);
	}

	/**
	 * Get Order Reference Details request sample
	 * 
	 * @param service
	 *            instance of OffAmazonPaymentsService service
	 * @param request
	 *            Action to invoke
	 */
	public static GetOrderReferenceDetailsResponse invokeGetOrderReferenceDetails(
			OffAmazonPaymentsService service,
			GetOrderReferenceDetailsRequest request, PrintWriter outStream) {
		try {
			GetOrderReferenceDetailsResponse response = service
	...
	/**
	 * Method to print Get Order Reference Details response to console
	 * 
	 * @param response
	 *            instance of GetOrderReferenceDetailsResponse
	 */
	public static void printResponse(GetOrderReferenceDetailsResponse response,
			PrintWriter outStream) {

		int tabLevel = 1;
		outStream.println("GetOrderReferenceDetails Action Response");
		outStream
				.println("=============================================================================");
		outStream.println();

		Utilities.printVariable("GetOrderReferenceDetailsResponse", null,
				tabLevel, outStream);
		tabLevel++;
		if (response.isSetGetOrderReferenceDetailsResult()) {
			Utilities.printVariable("GetOrderReferenceDetailsResponse", null,
					tabLevel, outStream);
			GetOrderReferenceDetailsResult getOrderReferenceDetailsResult = response
					.getGetOrderReferenceDetailsResult();
			tabLevel++;
			if (getOrderReferenceDetailsResult.isSetOrderReferenceDetails()) {
				Utilities.printVariable("OrderReferenceDetails", null,
						tabLevel, outStream);
				OrderReferenceDetails orderReferenceDetails = getOrderReferenceDetailsResult
						.getOrderReferenceDetails();
				tabLevel++;
				if (orderReferenceDetails.isSetAmazonOrderReferenceId()) {
					Utilities.printVariable("AmazonOrderReferenceId",
				...
				if (orderReferenceDetails.isSetBuyer()) {
					...
					tabLevel++;
					if (buyer.isSetName()) {
						Utilities.printVariable("Name", buyer.getName(),
								tabLevel, outStream);
					}
					if (buyer.isSetEmail()) {
						Utilities.printVariable("Email", buyer.getEmail(),
								tabLevel, outStream);
					}
					if (buyer.isSetPhone()) {
						Utilities.printVariable("Phone", buyer.getPhone(),
								tabLevel, outStream);
					}
					tabLevel--;
				}
				if (orderReferenceDetails.isSetOrderTotal()) {
					Utilities.printVariable("OrderTotal", null, tabLevel,
							outStream);
					OrderTotal orderTotal = orderReferenceDetails
							.getOrderTotal();
					tabLevel++;
					if (orderTotal.isSetCurrencyCode()) {
						Utilities.printVariable("CurrencyCode",
								orderTotal.getCurrencyCode(), tabLevel,
								outStream);
					}
					if (orderTotal.isSetAmount()) {
						Utilities.printVariable("Amount",
								orderTotal.getAmount(), tabLevel, outStream);
					}
					tabLevel--;
				}
				if (orderReferenceDetails.isSetSellerNote()) {
					Utilities.printVariable("SellerNote",
							orderReferenceDetails.getSellerNote(), tabLevel,
							outStream);
				}
				if (orderReferenceDetails.isSetPlatformId()) {
					Utilities.printVariable("PlatformId",
							orderReferenceDetails.getPlatformId(), tabLevel,
							outStream);
				}
				if (orderReferenceDetails.isSetDestination()) {
					Utilities.printVariable("Destination", null, tabLevel,
							outStream);
					Destination destination = orderReferenceDetails
							.getDestination();
					tabLevel++;
					if (destination.isSetDestinationType()) {
						Utilities.printVariable("DestinationType",
								destination.getDestinationType(), tabLevel,
								outStream);
					}
					if (destination.isSetPhysicalDestination()) {
						Utilities.printVariable("PhysicalDestination", null,
								tabLevel, outStream);
						Address physicalDestination = destination
								.getPhysicalDestination();
						tabLevel++;
						if (physicalDestination.isSetName()) {
							Utilities.printVariable("Name",
									physicalDestination.getName(), tabLevel,
									outStream);
						}
						...
						if (physicalDestination.isSetCity()) {
							Utilities.printVariable("City",
									physicalDestination.getCity(), tabLevel,
									outStream);
						}
						if (physicalDestination.isSetCounty()) {
							Utilities.printVariable("County",
									physicalDestination.getCounty(), tabLevel,
									outStream);
						}
						...
						if (physicalDestination.isSetPhone()) {
							Utilities.printVariable("Phone",
									physicalDestination.getPhone(), tabLevel,
									outStream);
						}
						tabLevel--;
					}
				...
				if (orderReferenceDetails.isSetReleaseEnvironment()) {
					Utilities.printVariable("ReleaseEnvironment",
							orderReferenceDetails.getReleaseEnvironment()
									.value(), tabLevel, outStream);
				}
				if (orderReferenceDetails.isSetSellerOrderAttributes()) {
					Utilities.printVariable("SellerOrderAttributes", null,
							tabLevel, outStream);
					SellerOrderAttributes sellerOrderAttributes = orderReferenceDetails
							.getSellerOrderAttributes();
					tabLevel++;
					if (sellerOrderAttributes.isSetSellerOrderId()) {
						Utilities.printVariable("SellerOrderId",
								sellerOrderAttributes.getSellerOrderId(),
								tabLevel, outStream);
					}
					if (sellerOrderAttributes.isSetStoreName()) {
						Utilities.printVariable("StoreName",
								sellerOrderAttributes.getStoreName(), tabLevel,
								outStream);
					}
					if (sellerOrderAttributes.isSetOrderItemCategories()) {
						Utilities.printVariable("OrderItemCategories", null,
								tabLevel, outStream);
						OrderItemCategories orderItemCategories = sellerOrderAttributes
								.getOrderItemCategories();
						java.util.List<String> orderItemCategoryList = orderItemCategories
								.getOrderItemCategory();
						tabLevel++;
						for (String orderItemCategory : orderItemCategoryList) {
							Utilities.printVariable("OrderItemCategory",
									orderItemCategory, tabLevel, outStream);
						}
						tabLevel--;
					}
					if (sellerOrderAttributes.isSetCustomInformation()) {
						Utilities.printVariable("CustomInformation",
				...
				if (orderReferenceDetails.isSetOrderReferenceStatus()) {
					Utilities.printVariable("OrderReferenceStatus", null,
							tabLevel, outStream);
					OrderReferenceStatus orderReferenceStatus = orderReferenceDetails
							.getOrderReferenceStatus();
					tabLevel++;
					if (orderReferenceStatus.isSetState()) {
						Utilities.printVariable("State",
								orderReferenceStatus.getState(), tabLevel,
								outStream);
					}
					if (orderReferenceStatus.isSetLastUpdateTimestamp()) {
						Utilities.printVariable("LastUpdateTimestamp",
								orderReferenceStatus.getLastUpdateTimestamp()
										.toString(), tabLevel, outStream);
					}
					if (orderReferenceStatus.isSetReasonCode()) {
						Utilities.printVariable("ReasonCode",
								orderReferenceStatus.getReasonCode(), tabLevel,
								outStream);
					}
					if (orderReferenceStatus.isSetReasonDescription()) {
						Utilities.printVariable("ReasonDescription",
				...
				if (orderReferenceDetails.isSetConstraints()) {
					Utilities.printVariable("Constraints", null, tabLevel,
							outStream);
					Constraints constraints = orderReferenceDetails
							.getConstraints();
					java.util.List<Constraint> constraintList = constraints
							.getConstraint();
					tabLevel++;
					for (Constraint constraint : constraintList) {
						Utilities.printVariable("Constraint", null, tabLevel,
								outStream);
						tabLevel++;
						if (constraint.isSetConstraintID()) {
							Utilities.printVariable("ConstraintID",
									constraint.getDescription(), tabLevel,
									outStream);
						}
						if (constraint.isSetDescription()) {
							Utilities.printVariable("Description",
									constraint.getDescription(), tabLevel,
									outStream);
						}
						tabLevel--;
				...
				if (orderReferenceDetails.isSetCreationTimestamp()) {
					Utilities.printVariable("CreationTimestamp",
							orderReferenceDetails.getCreationTimestamp()
									.toString(), tabLevel, outStream);
				}
				if (orderReferenceDetails.isSetExpirationTimestamp()) {
					Utilities.printVariable("ExpirationTimestamp",
		...
		if (response.isSetResponseMetadata()) {
			Utilities.printVariable("ResponseMetadata", response
					.getResponseMetadata().toString(), tabLevel, outStream);
			ResponseMetadata responseMetadata = response.getResponseMetadata();
			tabLevel++;
			if (responseMetadata.isSetRequestId()) {
				Utilities.printVariable("RequestId",
						responseMetadata.getRequestId(), tabLevel, outStream);
			}
			tabLevel--;
		}
		tabLevel--;
	...
	public static void validateOrderReferenceIsInACorrectState(
			OrderReferenceDetails orderReferenceDetails, String expectedState) {
		String actualState = orderReferenceDetails.getOrderReferenceStatus()
				.getState();
		if (!StringUtils.equalsIgnoreCase(expectedState, actualState)) {
			throw new IllegalStateException("Error with order reference "
					+ orderReferenceDetails.getAmazonOrderReferenceId()
					+ " - state is " + actualState + " ,expected "
					+ expectedState);
		}
	}
